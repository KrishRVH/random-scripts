...
export function runTests(): void {
    const tests: Array<{ name: string; fn: () => void }> = [
       ...
       {
          name:
             'Short flag group with equals where last does not accept value: ' +
             'non-strict preserves value as positional',
          fn: () => {
       ...
       {
          name:
             'Short group with equals: last does not accept; attached negative ' +
             'preserved as positional (not re-parsed)',
          fn: () => {
             const args = ['-ab=-10', 'x'];
             const result = parseArgs(args);
       ...
       {
          name:
             'Strict: short group with equals; last is known but not value-accepting ' + 'throws the correct error',
          fn: () => {
             let thrown = false;
             try {
                parseArgs(['-ab=value'], {
       ...
       {
          name:
             'Short expects value; next "-Infinity" is not consumed and becomes ' + 'short flags (documented quirk)',
          fn: () => {
